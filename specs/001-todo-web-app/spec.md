...
3. **Given** 使用者在待辦事項清單頁面, **When** 使用者點擊「新增」按鈕但輸入框為空, **Then** 顯示友善的錯誤提示訊息,要求輸入待辦事項內容
...
4. **Given** 待辦事項處於編輯模式, **When** 使用者清空文字並嘗試儲存, **Then** 顯示錯誤提示訊息,不允許儲存空白內容
...
- 當使用者嘗試新增超長文字(如超過 500 字元)時,系統應該限制輸入長度或顯示警告訊息
- 當使用者嘗試新增包含特殊字元(如 HTML 標籤、JavaScript 程式碼)的待辦事項時,系統應該正確處理並防止安全漏洞(XSS)
- 當清單中沒有任何待辦事項時,應顯示友善的空白狀態訊息(如「目前沒有待辦事項,點擊上方新增您的第一個任務!」)
- 當網路連線中斷時,系統應顯示離線狀態提示,並在可能的情況下允許繼續操作(資料暫存於本地)
- 當本地儲存失敗時(如瀏覽器儲存空間已滿、隱私模式限制、儲存權限被拒絕),系統應顯示持續的警告橫幅通知使用者資料無法持久化,但仍允許繼續操作(資料僅在當前會話有效,重新整理後會遺失)
...
- **FR-002**: 系統必須顯示所有待辦事項的清單,依完成狀態分組顯示(未完成的在上方,已完成的在下方),兩個組別之間使用分隔線和組別標題(如「進行中」「已完成」)明確區隔,同一組內按新增時間倒序排列(最新的在最上方)
- **FR-003**: 系統必須允許使用者將待辦事項標記為已完成或未完成
- **FR-004**: 系統必須允許使用者編輯現有待辦事項的文字內容
- **FR-005**: 系統必須允許使用者刪除待辦事項,並在刪除前顯示確認對話框
- **FR-006**: 系統必須在使用者輸入空白待辦事項時顯示友善的錯誤訊息
- **FR-007**: 系統必須限制待辦事項文字長度上限為 500 字元
- **FR-008**: 系統必須持久化儲存所有待辦事項資料,確保重新載入頁面後資料不遺失。若本地儲存失敗(如儲存空間已滿、隱私模式限制),系統必須顯示持續的警告訊息通知使用者,並允許繼續操作(資料僅在當前會話期間有效)
- **FR-009**: 系統必須在每次操作成功後於畫面右上角顯示 Toast 通知訊息,訊息應在 3 秒後自動消失,無需使用者手動關閉
- **FR-010**: 系統必須防止 XSS 攻擊,正確處理使用者輸入的特殊字元
...
- **SC-007**: 應用程式在手機、平板、桌面裝置上都能正常使用且操作流暢(響應式設計)
