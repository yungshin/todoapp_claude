...
- 當使用者嘗試新增超長文字時,系統應該採用以下策略:
  - **即時驗證**: 輸入框使用 `maxlength="500"` HTML 屬性阻止輸入超過 500 字元
  - **字數提示**: 輸入框下方即時顯示字數統計 (例如: "245/500 字元")
  - **達到上限**: 當達到 500 字元時,輸入框下方顯示提示文字 "已達字數上限"
  - **編輯模式**: 編輯現有待辦事項時套用相同規則
...
- 當有大量待辦事項時,系統應保持良好效能:
  - **基本負載** (100 個項目): 頁面載入 < 2 秒,操作不應明顯延遲
  - **高負載** (500-1000 個項目): 頁面載入 < 3 秒,滾動與互動回應 < 100 毫秒
...
- **FR-007**: 系統必須限制待辦事項文字長度上限為 500 字元。實作方式:
  - 輸入框使用 HTML `maxlength="500"` 屬性防止超長輸入
  - 即時顯示字數統計 (格式: "當前字數/500 字元")
  - 達到上限時顯示提示訊息 "已達字數上限"
  - 後端驗證 (如有) 也應拒絕超過 500 字元的請求
- **FR-008**: 系統必須使用瀏覽器 **localStorage API** 持久化儲存所有待辦事項資料,確保重新載入頁面後資料不遺失。錯誤處理策略:
  - **儲存失敗情境**: 儲存空間已滿 (QuotaExceededError)、隱私模式限制、或儲存權限被拒絕
  - **警告橫幅**: 在頁面頂部顯示黃色背景警告橫幅 (包含 X 關閉按鈕)
  - **警告訊息**: "⚠️ 無法儲存資料，重新整理後將遺失所有變更"
  - **橫幅行為**: 可暫時關閉,但每次重新載入頁面時再次顯示 (使用 sessionStorage 記錄關閉狀態)
  - **降級模式**: 允許繼續操作,資料僅在當前會話期間有效 (存於記憶體)
...
- **待辦事項 (Todo Item)**: 代表使用者要完成的任務,包含以下屬性:
  - **id** (string, 必要): 唯一識別碼,使用 `crypto.randomUUID()` 生成,用於編輯與刪除操作
  - **text** (string, 必要): 文字描述,最少 1 字元,最多 500 字元
  - **completed** (boolean, 必要): 完成狀態,預設為 `false` (未完成)
  - **createdAt** (number, 必要): 建立時間戳記 (Unix milliseconds, `Date.now()`),用於同組內排序
  - **updatedAt** (number, 必要): 最後更新時間戳記,用於追蹤修改歷史與未來的同步衝突解決
...
- **SC-009**: 系統在高負載情境下仍保持流暢操作:
  - **壓力測試**: 載入 500-1000 個待辦事項時,頁面載入 < 3 秒
  - **互動效能**: 滾動、勾選、編輯等操作的回應時間 < 100 毫秒
  - **測試工具**: 使用 Chrome DevTools Performance Profiler 或 Lighthouse 測量
...
2. **資料儲存**: 採用瀏覽器 **localStorage API** 作為持久化儲存機制,不需要後端伺服器或資料庫。選擇 localStorage 的理由:
   - 簡單易用的同步 API,適合小型資料集 (預期最多 1000 個待辦事項)
   - 所有現代瀏覽器原生支援,無需額外 polyfill
   - 資料大小限制 (通常 5-10MB) 遠超本應用需求
   - 未來如需更大儲存空間或結構化查詢,可升級至 IndexedDB
